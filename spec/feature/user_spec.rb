...
RSpec.describe 'Edit User', type: :feature, js: true do
    scenario 'valid inputs' do
        visit '/profile'
        expect(page).to have_button('Edit', visible: true)
        find('[data-testid="editButton"]', wait: 5).click
        expect(page).to have_button('Save', wait: 5)
        
        #find('[id="name"]').set('Test User')
