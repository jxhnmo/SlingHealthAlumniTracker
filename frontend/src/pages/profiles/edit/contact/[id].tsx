...
const EditContactMethods: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchContactMethods = async () => {
      ...
      try {
        ...
        setContactMethods(
          data.filter(
            (contact: ContactMethod) => contact.user_id === Number(id)
          )
        );
      } catch (err) {
  ...
  const handleUpdate = async (
    contactId: number,
    updatedContact: ContactMethod
  ) => {
    setError("");
    try {
      const response = await fetch(
        `${API_BASE_URL}/contact_methods/${contactId}`,
        {
          method: "PUT",
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setNewContactMethod({ contact_type: "", info: "", link: false });
    } catch {
    ...
    <div>
        ...
        {contactMethods.map((contact) => (
          <div key={contact.id}>
            ...
            <textarea
              value={contact.info}
              onChange={(e) =>
                handleUpdate(contact.id, { ...contact, info: e.target.value })
              }
              className="block"
            />
      ...
      <div>
        ...
        <textarea
          ...
          onChange={(e) =>
            setNewContactMethod({ ...newContactMethod, info: e.target.value })
          }
          className="block"
        />
        <button onClick={handleCreate}>Add Contact Method</button>
      </div>
    </div>
  );
};
